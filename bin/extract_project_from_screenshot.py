...
#   /Users/<you>/Documents/claude_projects/whisper
...
# legit project, turning `.../some-project_upgrade 1.4.10` into
# the candidate `some-project_upgrade 1.4.10` — which then
...
_STOP = {
    ...
    # Personal — the OS username; replace with your own login if you want to
    # exclude it from project-name candidates. Read from $USER at import time
    # below so this list isn't hard-coded to one identity.
    # UI / app chrome words
...
_VISION_PROMPT = (
    ...
    "    GitHub, src, bin, lib, Documents, Downloads, Desktop, Users, $USER, "
...
def _exchange_copilot_session(gh_token: str) -> tuple[str, str] | None:
    ...
    expiry. The chat endpoint can be enterprise-specific (Copilot Enterprise
    tenants return a custom api.<tenant>.githubcopilot.com host) and MUST
    come from this exchange — don't hardcode it.
    """
    now = int(time.time())
    # Try cache first
    try:
...
def _validate_llm_project(
    ...
    'some-project_upgrade-1.4.10' (real folder exists without the
