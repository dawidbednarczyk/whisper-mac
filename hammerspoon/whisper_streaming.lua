...
local OVERLAY_CLEANUP_DELAY_S    = 0.7  -- Bug T1: was 0.4, raced with paste keystroke at +0.45s
...
-- F-Back-NoAutoPaste (2026-04-24): when true, skip the cmd+V keystroke after
-- transcription completes. Clipboard is still set, log hook still fires, the
-- record is still saved — only the auto-paste keystroke is suppressed. The
-- user pastes manually with a second back-button press (which falls into the
-- "no recording → paste latest quick" branch in whisper_mouse.lua) or ⌘V.
-- Set externally by whisper_mouse.lua before calling M.stop().
M.suppress_auto_paste = false
...
local session_started_epoch = 0   -- wall-clock epoch (for transcription log hook)
...
-- ============================================================================
-- CONTEXT CAPTURE (frontmost app + project hint at trigger time)
-- ============================================================================
-- Used by the transcription log hook so the comparison page can group
-- recordings by project. Best-effort — never throws, always returns two strings.

local function capture_context()
    local ok_app, app = pcall(function() return hs.application.frontmostApplication() end)
    local app_name = ""
    if ok_app and app then
        local n_ok, n = pcall(function() return app:name() end)
        if n_ok and n then app_name = n end
    end

    local title = ""
    if ok_app and app then
        local w_ok, win = pcall(function() return app:focusedWindow() end)
        if w_ok and win then
            local t_ok, t = pcall(function() return win:title() end)
            if t_ok and t then title = t end
        end
    end

    local project_hint = ""
    if app_name == "Cursor" or app_name == "Code" or app_name == "Visual Studio Code" then
        -- Cursor / VS Code title formats:
        --   "file — project"             (em-dash)
        --   "file - project - Cursor"
        local p = title:match(".+ — (.+)$")
        if not p then
            p = title:match(".+ %- (.+) %- " .. app_name .. "$")
        end
        if p then project_hint = p end
    elseif app_name == "iTerm2" then
        -- F-Cursor-Agent (2026-04-24): Cursor Agent CLI runs *inside* iTerm2
        -- so app_name reports as "iTerm2" not "Cursor Agent". Two improvements:
        --   1. Try to extract a project from the iTerm tab title — Cursor Agent
        --      sets meaningful tab titles via OSC sequences (e.g. "Graphics
        --      Document Share" or "<file> · main"). The cwd-based fallback
        --      below is fine when title is generic; but when title looks like
        --      a Cursor Agent task description it's the better hint.
        --   2. Fall back to AppleScript cwd of current session (existing logic).
        --
        -- Detection strategy for Cursor Agent context:
        --   * Title pattern "<something> · <branch>" (cursor agent footer)
        --   * Title contains "Cursor Agent" or "Composer"
        --   * Title is non-trivial (> 3 chars, not the user's $PS1)
        local cursor_agent_title = nil
        if title and title ~= "" then
            -- Pattern: "Project Name · branch" or "file · branch" — Cursor Agent
            -- uses middle-dot separator. Take the LHS as the project.
            local lhs = title:match("^([^·]+)%s*·%s*[%w%-_/.]+$")
            if lhs and #lhs > 1 then
                cursor_agent_title = lhs:gsub("^%s+", ""):gsub("%s+$", "")
            end
        end
        local ok, cwd = pcall(function()
            local _ok, res = hs.osascript.applescript(
                [[tell application "iTerm2" to tell current session of current window to return path]]
            )
            return _ok and res or nil
        end)
        local cwd_hint = ""
        if ok and cwd and cwd ~= "" then
            cwd_hint = cwd:match("([^/]+)/?$") or cwd
        end
        -- Prefer cwd_hint (most reliable when iTerm sets it). Fall back to
        -- title-derived hint if cwd is empty (e.g. SSH session, or AppleScript
        -- denied). Combine both when they disagree to give the user signal.
        if cwd_hint ~= "" and cursor_agent_title and cursor_agent_title ~= cwd_hint then
            project_hint = cwd_hint .. " (" .. cursor_agent_title .. ")"
        elseif cwd_hint ~= "" then
            project_hint = cwd_hint
        elseif cursor_agent_title then
            project_hint = cursor_agent_title
        end
    elseif app_name == "Google Chrome" or app_name == "Arc" or app_name == "Safari" then
        project_hint = title  -- page title is the most useful project hint for browsers
    else
        project_hint = title
    end

    return app_name, project_hint
end
...
function M.start()
    ...
    session_started_epoch = os.time()  -- wall-clock t0 for transcription log hook
    -- Capture context (frontmost app + project hint) for grouping on the log page.
    do
        local _ctx_app, _ctx_proj = capture_context()
        session_context = { app_name = _ctx_app or "", project_hint = _ctx_proj or "" }
    end
    -- Capture full desktop screenshot for vision-based project extraction (post-paste).
    -- Fire-and-forget so it doesn't block recording start. Path uses session_started_epoch
    -- so we can match it to the wav after stop (basenames are unknown at start).
    do
        local screenshot_dir = os.getenv("HOME") .. "/.whisper_log/screenshots"
        hs.fs.mkdir(screenshot_dir)  -- safe if exists
        local screenshot_path = string.format("%s/session_%d.jpg", screenshot_dir, session_started_epoch)
        session_context.screenshot_path = screenshot_path

        -- Pick the screen that will host the overlay (focused window's screen → mouse → primary).
        -- screencapture -D wants a CG display ID, which is exactly what hs.screen:id() returns.
        local target_screen
        do
            ...
            if #screens <= 1 then
                ...
                if not target_screen then
                    target_screen = hs.mouse.getCurrentScreen()
                end
                target_screen = target_screen or hs.screen.primaryScreen()
            end
        end

        local args = {"-x", "-t", "jpg", "-T", "0"}
        local display_id = target_screen and target_screen:id() or nil
        if display_id then
            table.insert(args, "-D")
            table.insert(args, tostring(display_id))
        end
        table.insert(args, screenshot_path)

        hs.task.new("/usr/sbin/screencapture", nil, args):start()
        log_event("screenshot_capture_started", {
            path        = screenshot_path,
            display_id  = display_id,
            screen_name = target_screen and target_screen:name() or "<nil>",
        })
    end
    dbg.startSession("streaming", {
        ...
        ctx_app       = session_context.app_name,
        ctx_project   = session_context.project_hint,
    })
...
function M.stop()
    ...
    -- mode: nil/"auto" → auto-paste (cmd+v). "manual" → notify only (clipboard set, user pastes).
    local function paste_and_cleanup(text, source_tag, mode)
        ...
        -- ── Transcription log hook (non-blocking) ───────────────────────────
        -- Find the WAV that whisper-stream just produced (newest in RECORDINGS_DIR
        ...
        pcall(function()
            ...
            for f in hs.fs.dir(RECORDINGS_DIR) do
                if f:sub(-4) == ".wav" then
                    local p = RECORDINGS_DIR .. "/" .. f
                    local a = hs.fs.attributes(p)
                    if a and a.modification and a.modification > newest_mtime then
                        newest_mtime = a.modification
                        newest_path = p
                    end
                end
            end
            if not newest_path then return end
            -- Sanity: must be at/after this session
            if session_started_epoch > 0 and newest_mtime + 5 < session_started_epoch then return end
            -- Strip the LLM-aware prefix (and the newline that follows it) so
            -- the page shows the actual transcription, not the boilerplate.
            local logged_text = text or ""
            if TRANSCRIPT_PREFIX and TRANSCRIPT_PREFIX ~= "" then
                local prefix_with_nl = TRANSCRIPT_PREFIX .. "\n"
                if logged_text:sub(1, #prefix_with_nl) == prefix_with_nl then
                    logged_text = logged_text:sub(#prefix_with_nl + 1)
                elseif logged_text:sub(1, #TRANSCRIPT_PREFIX) == TRANSCRIPT_PREFIX then
                    logged_text = logged_text:sub(#TRANSCRIPT_PREFIX + 1):gsub("^%s+", "")
                end
            end
            -- Write text to temp file
            local txt_path = os.tmpname() .. ".whisper_stream.txt"
            local fh = io.open(txt_path, "w")
            if not fh then return end
            fh:write(logged_text)
            fh:close()
            local t0 = session_started_epoch > 0 and session_started_epoch or os.time()
            local dur_ms = math.floor((dbg.t_ms() - session_started_ms))
            local _q = function(s) return "'" .. (s or ""):gsub("'", "'\\''") .. "'" end
            local cmd = string.format(
                "%s %d %s %s %d %s %s %s >/dev/null 2>&1 &",
                hook,
                t0,
                _q(newest_path),
                "ggml-base.en.bin",
                dur_ms,
                _q(txt_path),
                _q(session_context.app_name),
                _q(session_context.project_hint)
            )
            os.execute(cmd)
            log_event("transcription_log_hook_fired", {
                wav          = newest_path,
                t0           = t0,
                dur_ms       = dur_ms,
                txt_path     = txt_path,
                ...
                project_hint = session_context.project_hint,
            })
            -- Attach screenshot + run vision project extractor (same as instant_paste_live path)
            local basename = newest_path:match("([^/]+)$")
            local screenshot_path = session_context.screenshot_path or ""
            log_event("screenshot_post_hook_scheduling", {
                basename = basename or "<nil>",
                screenshot = screenshot_path,
                source = source_tag,
            })
            if basename and screenshot_path ~= "" then
                if not _G.__whisper_pending_extractors then _G.__whisper_pending_extractors = {} end
                local timer_key = basename .. "_pac"
                _G.__whisper_pending_extractors[timer_key] = hs.timer.doAfter(1.5, function()
                    local ok, err = pcall(function()
                        ...
                        if hs.fs.attributes(screenshot_path, "mode") == "file" then
                            ...
                            log_event("screenshot_extractor_launched", {
                                basename = basename, screenshot = screenshot_path,
                            })
                        else
                            log_event("screenshot_missing_at_extract", { path = screenshot_path })
                        end
                    end)
                    if not ok then
                        log_event("screenshot_post_hook_timer_error", { basename = basename, err = tostring(err) })
                    end
                    _G.__whisper_pending_extractors[timer_key] = nil
                end)
            end
        end)
        -- ────────────────────────────────────────────────────────────────────
        if mode == "manual" then
            -- DO NOT auto-paste. The user has almost certainly switched windows
            -- by the time the slow stages finish; pasting into the wrong app is
            -- worse than no paste. Just notify so they know ⌘V will give them
            -- the improved version.
            pcall(function()
                hs.notify.new({
                    title           = "Whisper: better transcription ready",
                    informativeText = "⌘V to paste improved version (" .. (source_tag or "improved") .. ")",
                    autoWithdraw    = true,
                    withdrawAfter   = 6,
                }):send()
            end)
            ...
            hs.timer.doAfter(0.3, function()
                if target_app then
                    ...
                    hs.timer.doAfter(0.15, function()
                        if M.suppress_auto_paste then
                            log_event("paste_keystroke_suppressed", { source = source_tag, site = "paste_and_cleanup" })
                        else
    ...
    if use_instant then
        ...
        hs.timer.doAfter(0.3, function()
            if target_app then
                ...
                hs.timer.doAfter(0.15, function()
                    if M.suppress_auto_paste then
                        log_event("instant_paste_keystroke_suppressed", {
                            target_app = target_app:name() or "<nil>",
                        })
                        return
                    end
                    -- Bug T1: wrap keystroke + emit event for diagnostics.
                    local front = hs.application.frontmostApplication()
                    local front_name = front and front:name() or "<nil>"
                    local ok, err = pcall(function()
                        hs.eventtap.keyStroke({"cmd"}, "v")
                    end)
                    log_event("instant_paste_keystroke", {
                        ok = ok,
                        err = tostring(err),
                ...
                log_event("instant_paste_keystroke_skipped", {reason = "no_target_app"})
            end
        end)
        -- Bug #1 (real root cause, fix #2 / rpi/transcription-pipeline-consistency):
        -- This instant-paste path bypasses paste_and_cleanup() entirely, so the
        -- transcription log hook (which lives inside paste_and_cleanup) never fires
        -- → records.json never gets the new recording → page never updates → the
        ...
        pcall(function()
            ...
            if not newest_path then return end
            if session_started_epoch > 0 and newest_mtime + 5 < session_started_epoch then return end
            -- Strip the LLM-aware prefix
            local logged_text = instant_text or ""
            if TRANSCRIPT_PREFIX and TRANSCRIPT_PREFIX ~= "" then
                local prefix_with_nl = TRANSCRIPT_PREFIX .. "\n"
                if logged_text:sub(1, #prefix_with_nl) == prefix_with_nl then
                    logged_text = logged_text:sub(#prefix_with_nl + 1)
            ...
            local txt_path = os.tmpname() .. ".whisper_instant.txt"
            local fh = io.open(txt_path, "w")
            if not fh then return end
            fh:write(logged_text)
            fh:close()
            ...
            log_event("transcription_log_hook_fired", {
                ...
                dur_ms       = dur_ms,
                txt_path     = txt_path,
                source       = "instant_paste_live",
                app_name     = session_context.app_name,
                project_hint = session_context.project_hint,
            })
            -- Attach screenshot + run vision project extractor (async, fire-and-forget).
            -- Wait 1.5s so add-quick has written the record before we update it.
            local basename = newest_path:match("([^/]+)$")
            local screenshot_path = session_context.screenshot_path or ""
            log_event("screenshot_post_hook_scheduling", {
                basename = basename or "<nil>",
        ...
        retranscribe_task = hs.task.new(
            ...
            function(exitCode, stdOut, stdErr)
                    ...
                    if f then
                        ...
                        if raw_text and raw_text ~= "" then
                            ...
                            if use_instant then
                                ...
                                -- Bug #1 fix (rpi/transcription-pipeline-consistency):
                                -- Fire the log hook in stage-update mode so the comparison
                                -- page sees the improved (medium.en) text immediately.
                                ...
                                pcall(function()
                                    ...
                                    if not newest_path then return end
                                    if session_started_epoch > 0 and newest_mtime + 5 < session_started_epoch then return end
                                    -- v3: medium.en retranscribe is paste-only.
                                    -- The transcription log only tracks base_raw + large_corr stages.
                                    -- The background worker will fill large_corr later (via Copilot LLM
                                    -- + dictionary corrections in transcription_worker.py).
                                    log_event("instant_retranscribe_done_no_stage_log", {
                                        wav   = newest_path,
                                        chars = #(improved or ""),
                                    })
                                end)
                                ...
                                overlay.update(after_dict, "✓ Retranscribed (⌘V to paste)")
                                paste_and_cleanup(improved, "retranscribe", "manual")
                            end
                            return
                        else
                ...
                if use_instant then
                    ...
                    paste_and_cleanup(wrap_transcript(fallback), "fallback_live_text", "manual")
                end
            end,
...
-- F-MiddleOnlyStrict (2026-04-24): Read-only getters so whisper_mouse.lua can
-- (a) target the right app for paste even after a long wait, and
-- (b) compute "elapsed since stop" for the in-progress alert.
function M.getTargetApp()
    return target_app
end

function M.getSessionStartedEpoch()
    return session_started_epoch
end
