...
-- Provides a floating semi-transparent panel at top-center of the active screen
--
-- Screen selection:
--   • One display  → use that display (built-in only, or clamshell + one external).
--   • Multiple     → use the screen of the frontmost/focused window (terminal / IDE),
--                    so the overlay stays with where you are typing — not always the
--                    “primary” monitor, which is often the wrong one on a laptop+dock.
...
--- Pick which `hs.screen` should host the overlay.
-- One screen: that screen. Multiple: focused window's screen, else mouse, else primary.
local function screenForOverlay()
    ...
    local ok, mouseScr = pcall(function()
        return hs.mouse.getCurrentScreen()
    end)
    if ok and mouseScr then
        return mouseScr
    end
    return hs.screen.primaryScreen()
end

--- Frame for the webview on a given screen (top band, 80% width).
local function overlayFrameOnScreen(screen)
...
function M.create()
    ...
    local screen = screenForOverlay()
    if not screen then
        hs.showError("Could not determine screen for whisper overlay")
        return false
    end

    local overlayFrame, screenFrame = overlayFrameOnScreen(screen)

    if isCreated and webview then
        ev("create_reposition", {
            screen = screen:name() or "?",
            x = overlayFrame.x,
            y = overlayFrame.y,
        })
        pcall(function()
            webview:frame(overlayFrame)
        end)
        webview:show()
        isShown = true
        return true
